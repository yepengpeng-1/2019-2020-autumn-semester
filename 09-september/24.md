...
（还要讲文件系统啊）![](https://raw.githubusercontent.com/yuetsin/private-image-repo/master/2019/09/24-10-02-33-2019-09-24-10-02-23-image.png)

首先我们不要忘记，FileSystem 始终是基于我们的 Disk 的。HDD 也好 SSD 也好，都是 FS 的基础。
...
但是 HDD 最差的性能就是 RANDOM ACCESS TIME。也就是**任意读取性能**是很差的。
...
![](https://raw.githubusercontent.com/yuetsin/private-image-repo/master/2019/09/24-10-30-30-2019-09-24-10-30-28-image.png)

相对于原版 UNIX FileSystem，进行了这些优化：

1. 用类似于 ICS Chapter 9 的 Header/Footer，Bitmap，用字节位来记录是否 occupied。

2. 在每个文件的末尾处预留 10% 的空间，保证文件可以尽量连续保存。

3. Skip-sector positioning（稍后再谈）

#### FFS 规避 Rotational Delay

HDD 在连续读的时候比任意读的速度更快一些（也就是一些）

然而因为磁盘可能一直在转，我们在连续读取 Block 之间可能磁头就已经转走了…

因此我们可以用以下两种方案来解决这个问题：

1. 干脆不要连续读了，每次隔一点时间再读

2. 一次读两个 Block（硬件支持或者软件实现都可以）

![](https://raw.githubusercontent.com/yuetsin/private-image-repo/master/2019/09/24-10-39-51-2019-09-24-10-39-49-image.png)

> Cylinder Group Diagram

#### FFS 采用 First-fit Block Allocation
...
![](https://raw.githubusercontent.com/yuetsin/private-image-repo/master/2019/09/24-10-45-19-2019-09-24-10-45-17-image.png)
