...
public:
    ...
    OperInstr( std::string assem, TEMP::TempList* dst, TEMP::TempList* src, Targets* jumps ) : Instr( OPER ), assem( assem ), dst( dst ), src( src ), jumps( jumps ) {}
...
public:
    ...
    LabelInstr( std::string assem, TEMP::Label* label ) : Instr( LABEL ), assem( assem ), label( label ) {}
...
public:
    ...
    MoveInstr( std::string assem, TEMP::TempList* dst, TEMP::TempList* src ) : Instr( MOVE ), assem( assem ), dst( dst ), src( src ) {}
...
public:
    ...
    Proc( std::string prolog, InstrList* body, std::string epilog ) : prolog( prolog ), body( body ), epilog( epilog ) {}
};
