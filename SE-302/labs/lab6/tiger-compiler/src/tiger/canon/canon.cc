...
T::Stm* reorder( C::ExpRefList* rlist ) {
    ...
    // while ( rlist_backup ) {
    //     std::cout << "input linked list: #" << rlist_backup->head << std::endl;
    //     rlist_backup = rlist_backup->tail;
    // }
    // std::cout << "=== rlist print over ===" << std::endl;
    // std::cout << "well..." << std::endl;
    // std::cout << "checking condition." << std::endl;
    // std::cout << "rlist = " << rlist << std::endl;
    // std::cout << "(*rlist->head) = " << *rlist->head << std::endl;
    if ( !rlist ) {
        // std::cout << "[canon] [reoder] traverse nop statement" << std::endl;
        return new T::ExpStm( new T::ConstExp( 0 ) ); /* nop */
    }
    else if ( ( *rlist->head )->kind == T::Exp::Kind::CALL ) {
        // std::cout << "[canon] [reoder] T::Exp::Kind::CALL" << std::endl;
        TEMP::Temp* t = TEMP::Temp::NewTemp();
        *rlist->head  = new T::EseqExp( new T::MoveStm( new T::TempExp( t ), *rlist->head ), new T::TempExp( t ) );
        return reorder( rlist );
    }
    else {
        // std::cout << "[canon] [reoder] else branch" << std::endl;
        C::StmAndExp hd = do_exp( *rlist->head );
        T::Stm*      s  = reorder( rlist->tail );
        if ( commute( s, hd.e ) ) {
            // std::cout << "commute done" << std::endl;
            // std::cout << "rlist->head" << rlist->head << std::endl;
            *rlist->head = hd.e;

            auto ret_s = seq( hd.s, s );
            // std::cout << "ok, then" << std::endl;
            return ret_s;
        }
        else {
            // std::cout << "commute fail" << std::endl;
            TEMP::Temp* t = TEMP::Temp::NewTemp();
...
T::Stm* do_stm( T::Stm* stm ) {
    return stm->Canon( stm );
}

C::StmAndExp do_exp( T::Exp* exp ) {
    return exp->Canon( exp );
}
...
C::StmListList* next( T::StmList* prevstms, T::StmList* stms, TEMP::Label* done ) {
    if ( !stms )
        return next( prevstms, new T::StmList( new T::JumpStm( new T::NameExp( done ), new TEMP::LabelList( done, nullptr ) ), nullptr ), done );
    if ( stms->head->kind == T::Stm::Kind::JUMP || stms->head->kind == T::Stm::Kind::CJUMP ) {
        ...
        return next( prevstms, new T::StmList( new T::JumpStm( new T::NameExp( lab ), new TEMP::LabelList( lab, nullptr ) ), stms ), done );
    }
    else {
        prevstms->tail = stms;
...
C::StmListList* mk_blocks( T::StmList* stms, TEMP::Label* done ) {
    ...
    if ( stms->head->kind != T::Stm::Kind::LABEL ) {
        return mk_blocks( new T::StmList( new T::LabelStm( TEMP::NewLabel() ), stms ), done );
    }
...
void trace( T::StmList* list ) {
    ...
    if ( s->kind == T::Stm::Kind::JUMP ) {
        ...
        // std::cout << "GOing to call Look! jumpstm->jumps->head = " << jumpstm->jumps->head << std::endl;
    ...
    /* we want false label to follow CJUMP */
        ...
        if ( !cjumpstm )
            assert( 0 );
        // std::cout << "GOing to call Look! truelist = " << cjumpstm->true_label << ", falselist = " << cjumpstm->false_label << std::endl;
        ...
        if ( falselist ) {
            ...
            last->tail->head = new T::CjumpStm( T::notRel( cjumpstm->op ), cjumpstm->left, cjumpstm->right, cjumpstm->false_label, cjumpstm->true_label );
            last->tail->tail = truelist;
            trace( truelist );
        }
        else {
            TEMP::Label* falselabel = TEMP::NewLabel();
            last->tail->head        = new T::CjumpStm( cjumpstm->op, cjumpstm->left, cjumpstm->right, cjumpstm->true_label, falselabel );
            last->tail->tail        = new T::StmList( new T::LabelStm( falselabel ),
                                               new T::StmList( new T::JumpStm( new T::NameExp( cjumpstm->false_label ), new TEMP::LabelList( cjumpstm->false_label, nullptr ) ), get_next() ) );
        }
    }
    else
        assert( 0 );
