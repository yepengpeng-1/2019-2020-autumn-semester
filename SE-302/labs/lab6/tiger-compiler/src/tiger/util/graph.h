...
private:
    ...
    NodeList< T >* preds_;
    T*             info_;
    Node< T >() : mygraph_( nullptr ), mykey_( 0 ), succs_( nullptr ), preds_( nullptr ), info_( nullptr ) {}
};
...
template < class T > Node< T >* Graph< T >::NewNode( T* info ) {
    Node< T >*     n = new Node< T >();
...
template < class T > void Graph< T >::RmEdge( Node< T >* from, Node< T >* to ) {
    assert( from && to );
...
template < class T > int Node< T >::Degree() {
    return InDegree() + OutDegree();
}

template < class T > NodeList< T >* Node< T >::Adj() {
    return NodeList< T >::CatList( this->succs_, this->preds_ );
}

template < class T > NodeList< T >* Node< T >::Succ() {
    return this->succs_;
}

template < class T > NodeList< T >* Node< T >::Pred() {
    return this->preds_;
}

template < class T > T* Node< T >::NodeInfo() {
    return this->info_;
}

template < class T > int Node< T >::Key() {
    return this->mykey_;
}

template < class T > NodeList< T >* Graph< T >::Nodes() {
    return this->mynodes;
}

/* return true if a is in l list */
template < class T > bool NodeList< T >::InNodeList( Node< T >* a ) {
    NodeList< T >* p;
...
template < class T > NodeList< T >* NodeList< T >::DeleteNode( Node< T >* a, NodeList< T >* l ) {
    assert( a && l );
    if ( a == l->head )
        return l->tail;
    else
...
template < class T > NodeList< T >* NodeList< T >::CatList( NodeList< T >* a, NodeList< T >* b ) {
    if ( a == nullptr )
        return b;
    else
...
template < typename T, typename ValueType > using Table = TAB::Table< Node< T >, ValueType >;

/*
 * Print a human-readable dump for debugging.
 */
template < class T > void Graph< T >::Show( FILE* out, NodeList< T >* p, void showInfo( T* ) ) {
