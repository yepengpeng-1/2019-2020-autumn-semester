...
rm ./build/canoned.s ./build/uncanoned.s ./build/regalloc.txt ./build/unallocated.s
...
for tcase in $(ls $TESTCASEDIR/ -r); do
    if [ ${tcase##*.} = "tig" ]; then
        tfileName=${tcase##*/}
        ./$BIN $TESTCASEDIR/$tfileName &>my_output_${tfileName}.log
        gcc -Wl,--wrap,getchar -m64 $TESTCASEDIR/${tfileName}.s $RUNTIMEPATH -o test.out
        if [ ! -s test.out ]; then
