...
public:
    CompoundStm( Stm* stm1, Stm* stm2 ) : Stm( COMPOUND_SMT ), stm1( stm1 ), stm2( stm2 ) {}
...
public:
    OpExp( Exp* left, BinOp oper, Exp* right ) : Exp( OP_EXP ), left( left ), oper( oper ), right( right ) {}
    // TODO: you'll have to add some definitions here (lab1).
    int          MaxArgs() const override;
    IntAndTable* Interp( Table* ) const override;
...
public:
    ...
    virtual Table*       prInterp( Table* ) const = 0;
...
public:
    PairExpList( Exp* head, ExpList* tail ) : ExpList( PAIR_EXP_LIST ), head( head ), tail( tail ) {}
    // TODO: you'll have to add some definitions here (lab1).
    int          MaxArgs() const override;
    int          NumExps() const override;
    IntAndTable* Interp( Table* ) const override;
    Table*       prInterp( Table* ) const override;
...
class Table {
public:
    Table( std::string id, int value, const Table* tail ) : id( id ), value( value ), tail( tail ) {}
