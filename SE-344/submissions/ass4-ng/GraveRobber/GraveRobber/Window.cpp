...
void Window::render(GLfloat* vertexBuff, GLuint buffLen)
{
    ...
    /* 创建'物体'着色器 对象 */
    Shader objectShader(fullPath("./data/objectVS.glsl").c_str(), fullPath("./data/objectFS.glsl").c_str());
    /* 创建'光源'着色器 对象 */
    Shader lightShader(fullPath("./data/lightVS.glsl").c_str(), fullPath("./data/lightFS.glsl").c_str());
    ...
    /* ================ 设置材质属性 ================ */
    /* 漫反射纹理 */
    Texture diffuseTexture(fullPath("./data/title.jpg").c_str());
    /* (黑白)镜面光纹理 */
//    Texture specularTexture(fullPath("../../../../Resources/TextureImage/container_specular.png").c_str());
    /* (彩色)镜面光纹理 */
    Texture specularTexture(fullPath("./data/title_MIRROR.jpg").c_str());
